...
export default function NavigationButtons({
  ...
  const t = useTranslations("NaturalForm.Navigation");
  return (
    <div className="flex items-center justify-between pt-6 border-t border-zinc-800/40 mt-8 font-sans">
      <div>
        {lastSaved && (
          <button
            type="button"
            onClick={onClearDraft}
            className="text-xs text-red-400/80 hover:text-red-400 transition font-medium tracking-wide underline cursor-pointer"
          >
            {t("ClearDraft")}
          </button>
        )}
